...
-(NSURLSessionDataTask *)getBindedDeviceListWithUid:(NSString *)uid success:(void (^)(NSArray<MyDevice *> * _Nonnull, NSArray * _Nonnull))success error:(void (^)(NSError * _Nonnull))errorBlock failure:(void (^)(NSError * _Nonnull))failure
{
    uid = uid ?: @"";
    return [self POST:@"adminlock/edit/getAdminDevlistAndModel"/*@"adminlock/edit/getAdminDevlist"*/ parameters:@{@"user_id" : uid} success:^(id  _Nullable responseObject) {
        if (![responseObject isKindOfClass:NSDictionary.class])
        {
            !errorBlock ?: errorBlock([NSError errorWithDomain:@"服务器返回值错误" code:(int)KDSHttpErrorReturnValueIncorrect userInfo:nil]);
            return;
        }
        NSArray<MyDevice *> *devices = [MyDevice mj_objectArrayWithKeyValuesArray:[responseObject valueForKey:@"devList"]];
        NSArray * productInfoListArr = [KDSProductInfoList mj_objectArrayWithKeyValuesArray:[responseObject valueForKey:@"productInfoList"]];
        ...
        !success ?: success(devices.copy,productInfoListArr);
    } error:errorBlock failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        !failure ?: failure(error);
    }];
}
