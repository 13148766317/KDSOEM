...
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"ftpsuccess"  object:nil];//刷新设备状态
                NSLog(@"--ftp---stop2");
            }
            else {
                CFStreamError error = CFReadStreamGetError(stream);
...
- (void)setLocalFileURL:(NSURL *)localFileURL
{
    ...
    //xxx/Documents/orangecat-20190404/1554359778_picture.jpg
    NSString * localFilePath = self.localFileURL.absoluteString.stringByDeletingScheme;
    //xxx/Documents/orangecat-20190404
    NSString * localPath;
    if ([localFilePath containsString:@"jpg"]) {
        localPath = [localFilePath substringToIndex:localFilePath.length-23];
    }else if ([localFilePath containsString:@"raw"]){
        localPath = [localFilePath substringToIndex:localFilePath.length-21];
    }else if ([localFilePath containsString:@"h264"]){
        localPath = [localFilePath substringToIndex:localFilePath.length-22];
    }
...
- (void)stop
{
    ...
    CFWriteStreamSetProperty(self.writeStream, kCFStreamPropertyFileCurrentOffset, kCFBooleanFalse);
    CFWriteStreamSetClient(self.writeStream,
                           kCFStreamEventNone |
                           kCFStreamEventOpenCompleted |
                           kCFStreamEventHasBytesAvailable |
                           kCFStreamEventCanAcceptBytes |
