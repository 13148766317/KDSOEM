...
- (void) parseFirmwareFileWithName_v1:(NSString *)fileName path:(NSString *)filePath onFinish:(void(^)(NSMutableDictionary *header, NSDictionary *appInfo, NSArray *rowData, NSError *error))finish
{
    ...
    NSString *fileContents = [NSString stringWithContentsOfFile:[NSString pathWithComponents:[NSArray arrayWithObjects:filePath, fileName, nil]]
                                                        encoding:NSUTF8StringEncoding error:nil];
    NSLog(@"--{Kaadas}--filePath===%@, fileName===%@",filePath, fileName);

    NSLog(@"--{Kaadas}--fileContents===%@",fileContents);
    if (fileContents && fileContents.length > 0)
    {
        //Separate by new line
        ...
        if (fileContentsArr)
        {
            NSLog(@"--{Kaadas}--fileContents===%@",fileContentsArr);
            ...
            if (fileHeader.length >= FILE_HEADER_MAX_LENGTH_V1)
            {
                ...
                {
                    NSLog(@"--{Kaadas}--FILE_PARSER_ERROR_CODE==%@",fileContentsArr);
                    [MBProgressHUD showError:LOCALIZEDSTRING(@"unsupportedFileVersion")];
...
//            }
        ...
        {
            NSLog(@"--{Kaadas}--fileContents=error==%@",fileContentsArr);
            [MBProgressHUD showError:LOCALIZEDSTRING(@"parsingFailed")];
...
//            finish(nil, nil, nil, error);
    ...
    {
        NSLog(@"--{Kaadas}--FILE_EMPTY_ERROR");
        [MBProgressHUD showError:LOCALIZEDSTRING(@"FILE_EMPTY_ERROR")];
