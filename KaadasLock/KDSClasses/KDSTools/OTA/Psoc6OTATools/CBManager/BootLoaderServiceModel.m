...
/*!
 *  @method discoverService:CharacteristicsWithCompletionHandler:
 *
 *  @discussion 传递发现的service给CyCBManager.
 *
 */
-(void) discoverService:(NSArray *)services peripheral:(CBPeripheral *)peripheral CharacteristicsWithCompletionHandler:(void (^) (BOOL success, NSError *error)) handler
{
    cbCharacteristicDiscoverHandler = handler;
    NSLog(@"--{Kaadas}--peripheral=%@",peripheral);
    
    [[CyCBManager sharedManagerWithPeripheral:peripheral] setCbCharacteristicDelegate:self];

    NSLog(@"--{Kaadas}--CBservice=%@",services);
    for (CBService *service in services) {
        
        if ([service.UUID isEqual:CUSTOM_BOOT_LOADER_SERVICE_UUID])
        {
            NSLog(@"--{Kaadas}--service.UUID=%@",service.UUID);
            for (CBCharacteristic *characteristic in service.characteristics)
            {
                NSLog(@"--{Kaadas}--characteristic.UUID=%@",characteristic.UUID);
...
{
    NSLog(@"--{Kaadas}--command111=%@",bootloaderCharacteristic);
    cbBootloaderCharacteristicNotificationHandler = handler;
    NSLog(@"--{Kaadas}--command222=%@",bootloaderCharacteristic);
...
//        [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.service.UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:bootloaderCharacteristic.UUID] descriptor:nil operation:START_NOTIFY];
        [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:bootloaderCharacteristic.UUID] descriptor:nil operation:START_NOTIFY];
...
{
    NSLog(@"--{Kaadas}--commandCode=%hu",commandCode);
    NSLog(@"--{Kaadas}--data=%@",data);
...
//        NSString * serviceName = [ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.service.UUID];
        NSString * serviceName = [ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.UUID];
        ...
        if (self.isWriteWithoutResponseSupported)
        {
            ...
            do
            {
                ...
                NSLog(@"--{Kaadas}--localData11=%@",localData);
                NSLog(@"--{Kaadas}--myPeripheral222=%@",[[CyCBManager sharedManager] myPeripheral]);
        ...
        {
            NSLog(@"--{Kaadas}--data111=%@",data);
            NSLog(@"--{Kaadas}--myPeriphera111l=%@",[[CyCBManager sharedManager] myPeripheral]);
...
//        [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.service.UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:bootloaderCharacteristic.UUID] descriptor:nil operation:STOP_NOTIFY];
        [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:bootloaderCharacteristic.UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:bootloaderCharacteristic.UUID] descriptor:nil operation:STOP_NOTIFY];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    if (error) {
        KDSLog(@"--{Kaadas}--向蓝牙设备发送数据失败=%@",error.localizedDescription);
    }else{
        KDSLog(@"--{Kaadas}--向蓝牙设备发送数据成功 ");
    }
}
...
{
    NSLog(@"--{Kaadas}--CBservice=%@",service);
...
//    if (commandArray) {
//        NSLog(@"--{Kaadas}--回复=%@",[commandArray objectAtIndex:0]);
//    }
//    NSLog(@"--{Kaadas}--回复error=%@",error);
    ...
    if (error == nil) {
        if ([characteristic.UUID isEqual:BOOT_LOADER_CHARACTERISTIC_UUID]) {
            ...
            if (iFileVersionTypeCYACD2 == self.fileVersion) {
                NSLog(@"--{Kaadas}--回复otaError11=%c",otaError);
                ...
                // Checking the error code from the response
                NSLog(@"--{Kaadas}--回复otaError22=%c",otaError);
