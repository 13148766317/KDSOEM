        ...
        if ([controller isKindOfClass:[KDSAddBleAndWiFiLockStep1 class]]) {
            ...
            for (int i = 0; i < wifiLockModel.count; i ++) {
                ///[string caseInsensitiveCompare:string2] == NSOrderedSame
                 if ([self.lock.wifiDevice.productModel caseInsensitiveCompare:wifiLockModel[i]] == NSOrderedSame) {
                   //新的添加Wi-Fi锁流程
                   KDSDeviceConnectionStep1VC * vc = [KDSDeviceConnectionStep1VC new];
                   [self.navigationController pushViewController:vc animated:YES];
                     return;
            ...
            for (int k = 0; k < bleWifiLockModel.count; k ++) {
                if ([self.lock.wifiDevice.productModel caseInsensitiveCompare:bleWifiLockModel[k]] == NSOrderedSame) {
                    //ble+Wi-Fi配网
                    KDSBleAndWiFiSearchBluToothVC *A = [KDSBleAndWiFiSearchBluToothVC new];
                    [self.navigationController pushViewController:A animated:YES];
                    return;
