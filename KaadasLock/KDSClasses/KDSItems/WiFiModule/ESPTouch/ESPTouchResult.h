...
/// it is used to check whether the esptouch task is executed suc它用于检查esptouch任务是否成功执行
@property (nonatomic,assign) BOOL isSuc;

/// it is used to store the device's bssid
@property (nonatomic,strong) NSString * bssid;

/// it is used to check whether the esptouch task is cancelled by user用于检查用户是否取消了ESPtouch任务
